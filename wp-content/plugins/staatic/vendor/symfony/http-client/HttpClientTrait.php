...
trait HttpClientTrait
{
    ...
    private static function prepareRequest(?string $method, ?string $url, array $options, array $defaultOptions = [], bool $allowExtraOptions = \false): array
    {
        ...
        if (isset($options['body'])) {
            if (\is_array($options['body']) && (!isset($options['normalized_headers']['content-type'][0]) || strpos($options['normalized_headers']['content-type'][0], 'application/x-www-form-urlencoded') === false)) {
    ...
    private static function removeDotSegments(string $path)
    {
        ...
        while (!\in_array($path, ['', '.', '..'], \true)) {
            if ('.' === $path[0] && (strncmp($path, $p = '../', strlen($p = '../')) === 0 || strncmp($path, $p = './', strlen($p = './')) === 0)) {
                $path = substr($path, \strlen($p));
            } elseif ('/.' === $path || strncmp($path, '/./', strlen('/./')) === 0) {
